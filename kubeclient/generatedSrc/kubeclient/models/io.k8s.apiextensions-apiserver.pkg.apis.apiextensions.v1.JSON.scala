...
object JSON {
  implicit val `io.k8s.apiextensions-apiserver.pkg.apis.apiextensions.v1.JSON-Decoder`
      : Decoder[JSON] =
    decodeJson.map(json => JSON(json))

  implicit val `io.k8s.apiextensions-apiserver.pkg.apis.apiextensions.v1.JSON-Encoder`
      : Encoder[JSON] =
