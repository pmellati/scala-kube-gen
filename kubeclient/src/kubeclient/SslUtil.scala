...
/** A very fragile set of utilities for handling SSL. */
object SslUtil {
  ...
  private def byteStream(s: String) = new ByteArrayInputStream(s.getBytes("UTF-8"))

  private def base64DecodedBytes(encodedData: String): Array[Byte] = {
    val singleLine = encodedData.split("\n").mkString
    Base64.getDecoder().decode(singleLine)
  }

  private def base64DecodedStr(encoded: String): String =
